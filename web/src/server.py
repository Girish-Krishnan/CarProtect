...
def send_data(req):
  ...
  db = mysql.connect(host=db_host, database=db_name, user=db_user, passwd=db_pass)
  cursor = db.cursor()
  cursor.execute("insert into Sensor_Data (username, password, pir, vib, ax, ay, az, lat, long, time) values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s);", (username, password, pir, vib, ax, ay, az, lat, long, str(datetime.now())))
  db.commit()
  db.close()
  return {}
...
def stream_data(req):
  ...
  db = mysql.connect(host=db_host, database=db_name, user=db_user, passwd=db_pass)
  cursor = db.cursor()
  cursor.execute("select pir,vib,ax,ay,az,lat,long from Sensor_Data where username = %s and password = %s order by timestamp DESC;", (username, password))
